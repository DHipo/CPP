...
Vector2 operator-(Vector2 &v1, Vector2 &v2){
    Vector2 rest= {(v1.x - v2.x),(v1.y - v2.y)};
    return rest;
}
...
void operator<<(std::ostream& cout, std::vector<int>& arr){
    cout<<"Arr: {";
    for(int i : arr) cout<<i<<",";
    cout<<"}";
}

std::ostream& operator<<(std::ostream& cout, CONSOLE_SIZE& conSize){
    return cout<<"Console size: [h:"<<conSize.height<<" w:"<<conSize.width<<"]"<<std::endl;
}
